$(document).ready(function() {
    ...
    var table = $('#datatable').DataTable({
        ...
        cache: false,
        ajax: {
            url: baseURL + '/super-admin/users/list',
            data: function (d) {
                d.role = $('#role').val();
            },
        },
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    if (userID != row.id && row.role !== 'super_admin') {
                        ...
                        var deleteButton = '<form class="d-inline" method="POST" action="' + baseURL + '/super-admin/users/' + row.id + '"><input type="hidden" name="_token" value="' + $('meta[name="csrf-token"]').attr('content') + '" /><input type="hidden" name="_method" value="DELETE" /><button type="submit" class="btn btn-sm btn-danger"><i class="fas fa-trash"></i> <span class="d-none d-lg-inline">Hapus</span></button></form>';
