...
$(document).ready(function() {
    ...
    $('#datatable').DataTable({
        ...
        cache: false,
        ajax: {
            url: baseURL + '/admin/designations/list',
        },
        columns: [
            {
                data: null,
            ...
            },
            {
                ...
                className: 'text-xl-center',
                render: function (data, type, row) {
                    var editButton = '<a href="' + baseURL + '/admin/designations/' + row.id + '/edit" class="btn btn-sm btn-warning"><i class="fas fa-edit"></i> <span class="d-none d-lg-inline">Ubah</span></a>';

                    var deleteButton = '<form class="d-inline" method="POST" action="' + baseURL + '/admin/designations/' + row.id + '"><input type="hidden" name="_token" value="' + $('meta[name="csrf-token"]').attr('content') + '" /><input type="hidden" name="_method" value="DELETE" /><button type="submit" class="btn btn-sm btn-danger"><i class="fas fa-trash"></i> <span class="d-none d-lg-inline">Hapus</span></button></form>';
